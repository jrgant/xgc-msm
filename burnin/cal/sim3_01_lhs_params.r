...
lhs_new_limits <- readRDS(here::here("inst", "cal", "sim2_sel_lhs_limits.rds"))
...
if (priornames_ok & priorvals_ok) {
  if (!file.exists(file.path(caldir, "sim3"))) {
    dir.create(file.path(caldir, "sim3"))
  }
  saveRDS(lhs_real, file = file.path(caldir, "sim3", "lhs_sim3.rds"))
} else {
...
saveRDS(.lec.Random.seed.table, file.path(caldir, "sim3", "seeds_sim3.rds"))
...
make_batch_script <- function(jobname, walltime, partition, mem,
  ...
  sdir <- here::here("burnin", "cal", "sim3")
  writeLines(
    text = specs,
    con = file.path(sdir, paste0("sim3_batch", batchid, ".sh"))
  )
