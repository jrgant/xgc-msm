...
lhs_new_limits <- readRDS(
  here::here("inst", "cal", "sim2_altrim_sel_lhs_limits.rds")
)
...
pvec2 <- function(name) {
  ...
  c("unif", tmp[, s3_ll], tmp[, s3_ul], name)
}
...
if (priornames_ok & priorvals_ok) {
  if (!file.exists(file.path(caldir, "sim3_altrim"))) {
    dir.create(file.path(caldir, "sim3_altrim"))
  }
  saveRDS(lhs_real, file = file.path(caldir, "sim3_altrim", "lhs_sim3_altrim.rds"))
} else {
...
saveRDS(
  ...
  file.path(caldir, "sim3_altrim", "seeds_sim3_altrim.rds")
)
...
make_batch_script <- function(jobname, walltime, partition, mem,
  ...
  sdir <- here::here("burnin", "cal", "sim3_altrim")
  writeLines(
    text = specs,
    con = file.path(sdir, paste0("sim3_altrim_batch", batchid, ".sh"))
  )
