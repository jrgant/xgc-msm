...
priors <- list(
  ...
  pvec(0, 5,  "RIM_RATE_MAIN"),
  pvec(0, 5,  "RIM_RATE_CASUAL"),
...
set.seed(5882300)
lhs_unif <- randomLHS(10000, length(priors))
...
if (priornames_ok & priorvals_ok) {
  if (!file.exists(file.path(caldir, "sim1_altrim"))) {
    dir.create(file.path(caldir, "sim1_altrim"))
  }
  saveRDS(
    lhs_real,
    file = file.path(caldir, "sim1_altrim", "lhs_sim1_altrim.rds")
  )
} else {
...
saveRDS(
  ...
  file.path(caldir, "sim1_altrim", "seeds_sim1_altrim.rds")
)
...
make_batch_script <- function(jobname, walltime, partition, mem,
  ...
  writeLines(
    ...
    con = file.path(
      sdir,
      paste0("sim1_altrim_batch", sprintf("%02d", batchid), ".sh")
    )
  )
