...
if (priornames_ok & priorvals_ok) {
  if (!file.exists(file.path(caldir, "sim2_altrim"))) {
    dir.create(file.path(caldir, "sim2_altrim"))
  }
  saveRDS(
    lhs_real,
    file = file.path(caldir, "sim2_altrim", "lhs_sim2_altrim.rds")
  )
} else {
...
saveRDS(
  ...
  file.path(caldir, "sim2_altrim", "seeds_sim2_altrim.rds")
)
...
make_batch_script <- function(jobname, walltime, partition, mem,
  ...
  specs <- paste(
    ...
    "Rscript ./burnin/cal/sim2_02_altrim_lhs.r --vanilla",
    sep = "\n"
  )

  sdir <- here::here("burnin", "cal", "sim2_altrim")
  writeLines(
    text = specs,
    con = file.path(sdir, paste0("sim2_altrim_batch", batchid, ".sh"))
  )
