...
.lec.Random.seed.table <- readRDS(
  here::here("inst", "analysis01_epi", "seeds_altrim.rds")
)
...
input_list <- readRDS(
  here::here("inst", "analysis01_epi", "paramset_sampled_altrim.rds")
)

selected_input <- input_list[[slurm_array_task_id]]

param_sets <- readRDS(here::here("inst", "cal", "altrim_analysis_inputs.rds"))
...
if (Sys.getenv("SIMDIR") == "") {
  outpath <- "~/scratch/analysis01_epi_altrim"
} else {
