...
selected_paramsets <-
  readRDS(here::here("inst", "cal", "altrim_analysis_inputs.rds"))
...
saveRDS(
  paramset_sampled,
  here::here(an01_path, "paramset_sampled_altrim.rds")
)
...
saveRDS(
  ...
  here::here(an01_path, "seeds_altrim.rds")
)
...
make_batch_script <- function(jobname, walltime, partition, mem,
  ...
  writeLines(
    text = specs,
    con = file.path(sdir, paste0("02_main_altrim", batchid, ".sh"))
  )

}

make_batch_script(
  jobname = "AltRim-Episim",
  walltime = "3:00:00",
  partition = "batch",
  mem = "3GB",
  ncores = 1,
  array = paste0("1-", nsim_spec),
  log_fullpath = "AltRim-MainSim_ARRAY-%A_JOB-%J_SIMNO-%4a.log",
  batchid = 1,
  nsims = 1,
  nsteps = 52 * 65,
  add_arrivals = 1.285,
  simdir = "~/scratch/AltRim_MainEpi"
)
