...
## check whether current run is burnin or an STI screening scenario:
...
if (Sys.getenv("STI_SCREEN_TYPE") == "burnin") {
  ...
  .lec.Random.seed.table <- readRDS(
    here::here("inst", an02_path, "seeds_burnin.rds")
  )
  ...
  idx <- which(
    as.numeric(
      str_extract(burnins, "(?<=episim_)[0-9]{4}")
    ) == slurm_array_task_id
  )

  est <- readRDS(burnins[idx])
...
input_list <- readRDS(
  here::here("inst", an02_path, "paramset_sampled.rds")
)
...
if (Sys.getenv("SIMDIR") == "") {
  outpath <- paste0("~/scratch/", an02_path)
} else {
...
param <- param_msm(
  ...
  a.rate            = netstats$demog$mortrate.marginal + arate_adjust / 20000,
  # urethral-to-rectal transmission probability
  u2rgc.tprob       = fmt_getenv("U2RGC_PROB"),
  # urethral-to-pharyngeal transmission probability
  u2pgc.tprob       = fmt_getenv("U2PGC_PROB"),
  # rectal-to-urethral transmission probability
  r2ugc.tprob       = fmt_getenv("R2UGC_PROB"),
  # pharyngeal-to-urethral transmission probability
...
starttime <- fmt_getenv("STARTTIME")
...
control <- control_msm(
  ...
  stiScreeningProtocol    = Sys.getenv("STI_SCREEN_TYPE"),
  skip.check              = TRUE,
  # FLAG: Kissing considered exposure for CDC protocol?
  cdcExposureSite_Kissing = as.logical(Sys.getenv("STI_SCREEN_KISS_EXPOSURE")),
  # NOTE Must set to avoid error from saveout.net()
  tergmLite               = TRUE,
  debug_stitx             = FALSE,
  save.network            = FALSE,
  verbose                 = FALSE
