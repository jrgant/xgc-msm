...
.lec.Random.seed.table <- readRDS(
  here::here("inst", an02_path, "seeds.rds")
)
...
control <- control_msm(
  ...
  skip.check              = TRUE,
  cdcExposureSite_Kissing = as.logical(Sys.getenv("STI_SCREEN_KISS_EXPOSURE")), # FLAG: Kissing considered exposure for CDC protocol?
